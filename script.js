...
async function initializeMap() {
  ...
  try {
    ...
    populationInfo.update = function (props) {
        // DIUBAH: Tidak perlu konversi, karena data sudah dalam jiwa/hektar
        const densityValue = props ? props.kep_pend : 0;
        this._div.innerHTML = '<h4>Kepadatan Penduduk</h4>' +  (props ?
            '<b>' + props.NAMOBJ + '</b><br />' + (densityValue || 0).toLocaleString('id-ID', {minimumFractionDigits: 1, maximumFractionDigits: 1}) + ' jiwa / hektar'
            : 'Arahkan ke salah satu wilayah');
    };
    
    // DIUBAH: Grade/rentang kelas disesuaikan untuk satuan jiwa/hektar
    function getColor(d) {
    ...
    function stylePopulation(feature) {
        // Data `kep_pend` langsung digunakan karena sudah dalam satuan yang benar
        return {
    ...
    legend.onAdd = function (map) {
        // DIUBAH: Cukup satu array `grades` dalam satuan hektar
    ...
    const overlayMaps = {
      ...
      "📍 Situs Sejarah": historicalSitesLayer,
